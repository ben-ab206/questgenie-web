...
import { exportQuestionsBankToCSV, exportQuestionsBankToPDF } from "@/lib/utils";
import { QuestionBank } from "@/types/questions";
...
const ExportDialog = ({ dialogOpen, data, onClose }: ExportDialogProps) => {
    ...
    const exportQuestionsWithFilename = (questions: QuestionBank[], filename?: string) => {
    ...
    const onExport = (type: "pdf" | "json" | "csv") => {
        ...
        if(type === "pdf") exportQuestionsBankToPDF(data, `testing-${timestamp}`)
        if(type === "csv") exportQuestionsBankToCSV(data, `testing-${timestamp}`)
        onClose();
    }
